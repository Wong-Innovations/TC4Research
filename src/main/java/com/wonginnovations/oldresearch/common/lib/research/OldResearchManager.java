...
public class OldResearchManager {
    ...
    public static boolean createClue(World world, EntityPlayer player, Object clue, AspectList aspects) {
        ...
        if(keys.size() > 0) {
            ...
            OldResearch.proxy.getOldResearchManager().completeResearch(player, "@" + key);
            return true;
        } else {
            return false;
        }
    ...
    public static ItemStack createNote(ItemStack stack, String key, World world) {
        ...
        if(primaryaspect == null) {
            ...
            HashMap<String, OldResearchManager.HexEntry> hexEntries = new HashMap<>();
            ...
            for(HexUtils.Hex hex : outerRing) {
                hexes.put(hex.toString(), hex);
                hexEntries.put(hex.toString(), new OldResearchManager.HexEntry(rr.tags.getAspects()[count], 1));
                ++count;
            }
